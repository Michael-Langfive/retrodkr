...
class PointerNet(nn.Module):
    ...
    def forward(self, embedded_inputs):
        ...
        Input: batch*sequence_lens*embedding_dim
        output:batch*token_lens*position_len
        pointers: the index
        eg: output[0]
        [0.2055, 0.2001, 0.2005, 0.2009, 0.1930],
        [0.0000, 0.2518, 0.2524, 0.2528, 0.2430],
        [0.0000, 0.3371, 0.3378, 0.0000, 0.3252],
        [0.0000, 0.5092, 0.0000, 0.0000, 0.4908],
...
if __name__ == "__main__":
    PNet = PointerNet(embedding_dim=64, hidden_dim=128, lstm_layers=3, dropout=0.2, bidir=False)
    scores = torch.randn((8, 5, 64)
    res1, res2 = PNet(scores)
