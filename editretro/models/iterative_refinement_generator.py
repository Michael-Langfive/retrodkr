...
class IterativeRefinementGenerator(object):
    ...
    def generate(self, models, sample, prefix_tokens=None):
        ...
        # === [新增] 获取 Ref, Frag, Sim ===
        # 使用 .get 避免在没有这些键时报错 (比如纯 inference 模式未提供额外数据)
        ref_tokens = sample['net_input'].get('ref_tokens', None)
        ref_lengths = sample['net_input'].get('ref_lengths', None)
        frag_tokens = sample['net_input'].get('frag_tokens', None)
        frag_lengths = sample['net_input'].get('frag_lengths', None)
        sim_scores = sample['net_input'].get('sim_scores', None)
        # 组装 Encoder 输入 (顺序必须与 EditRetroEncoder.forward 一致)
        # EditRetroEncoder.forward(src, src_len, ref, ref_len, frag, frag_len, sim)
        encoder_inputs = [
        ...
        # initialize
        encoder_out = model.forward_encoder(encoder_inputs)
        if self.init_src:
