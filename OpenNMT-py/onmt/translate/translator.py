...
def build_translator(opt, report_score=True, logger=None, out_file=None):
    ...
    elif model_opt.model_task == ModelTask.RetroDKR:
        print("lambda:", model_opt.lambda1, model_opt.lambda2, model_opt.lambda3)
        # 使用RetroDKRTranslator
        translator = RetroDKRTranslator.from_opt(
            model,
            fields,
            opt,
            model_opt,
...
class Inference(object):
    ...
    def from_opt(
        ...
        sim_reader = None
        frag_reader = None
        if model_opt.model_task == ModelTask.RetroDKR:
            ref_reader = inputters.str2reader["text"].from_opt(opt)
            sim_reader = inputters.str2reader["text"].from_opt(opt)
            frag_reader = inputters.str2reader["text"].from_opt(opt)
        return cls(
            model,
            fields,
            src_reader,
    ...
    def translate(
        ...
        ref_data = {"reader": self.ref_reader, "data": ref, "features": {}} if ref else None
        sim_data = {"reader": self.sim_reader, "data": sim, "features": {}} if sim else None
        frag_data = {"reader": self.frag_reader, "data": frag, "features": {}} if frag else None

        # 配置数据集
        data_configs = [("src", src_data), ("tgt", tgt_data)]
        if ref_data:
            data_configs.append(("ref", ref_data))
        if sim_data:
            data_configs.append(("sim", sim_data))
        if frag_data:
            data_configs.append(("frag", frag_data))
        _readers, _data = inputters.Dataset.config(data_configs)
    ...
    def _report_score(self, name, score_total, words_total):
        if words_total == 0:
            ...
            print(f"Score details: total={score_total}, words={words_total}, avg={avg_score}")
    ...
    def report_results(
        ...
        results = {"predictions": decode_strategy.predictions, "scores": decode_strategy.scores,
                   "attention": decode_strategy.attention, "batch": batch, "gold_score": gold_score}
...
class RetroDKRTranslator(Inference):
    """
    Translator class for RetroDKR models handling source, reference and similarity scores.
    """

    @classmethod
    def validate_task(cls, task):
        """Validate that the task is supported by this translator"""
        if task != ModelTask.RetroDKR:
            raise ValueError(
                f"RetroDKRTranslator does not support task {task}."
                f" Tasks supported: {ModelTask.RetroDKR}"
            )
    ...
    def _align_forward(self, batch, predictions):
        ...
        n_best = batch_tgt_idxs.size(1)
        src, enc_states, memory_bank, src_lengths, ref_memory_bank, ref_lengths, \
            frag_memory_bank, frag_lengths, frag_sim = self._run_encoder(batch)
        src = tile(src, n_best, dim=1)
        enc_states = tile(enc_states, n_best, dim=1)

        def tile_memory_and_lengths(spc_memory_bank, lengths, n_best):
            if isinstance(spc_memory_bank, tuple):
                spc_memory_bank = tuple(tile(x, n_best, dim=1) for x in spc_memory_bank)
            else:
                spc_memory_bank = tile(spc_memory_bank, n_best, dim=1)
            lengths = tile(lengths, n_best)
            return spc_memory_bank, lengths

        memory_bank, src_lengths = tile_memory_and_lengths(memory_bank, src_lengths, n_best)
        ref_memory_bank, ref_lengths = tile_memory_and_lengths(ref_memory_bank, ref_lengths, n_best) \
            if ref_memory_bank is not None else (None, None)
        sim = batch.sim
        sim = tile(sim, n_best) if sim is not None else None
        frag_memory_bank, frag_lengths = tile_memory_and_lengths(frag_memory_bank, frag_lengths, n_best) \
            if frag_memory_bank is not None else (None, None)
        frag_sim = tile(frag_sim, n_best) if frag_sim is not None else None
        self.model.decoder.init_state(src, memory_bank, enc_states)
    ...
    def _run_encoder(self, batch):
        """Run encoder on source and reference sequences"""

        def encode_sequence(seq_data):
            seq, lengths = seq_data if isinstance(seq_data, tuple) else (seq_data, None)
            enc_states, memory_bank, lengths = self.model.encoder(seq, lengths)
            return enc_states, memory_bank, lengths

        enc_states, memory_bank, src_lengths = encode_sequence(batch.src)
        use_ref = hasattr(batch, 'ref') and self.model.decoder.lambda2 > 0.0
        use_frag = hasattr(batch, 'frag') and self.model.decoder.lambda3 > 0.0
        ref_memory_bank, ref_lengths, frag_memory_bank, frag_lengths, frag_sim = None, None, None, None, None
        if use_ref:
            _, ref_memory_bank, ref_lengths = encode_sequence(batch.ref)

        if use_frag:
            frag_emb, frag_memory_bank, frag_lengths = encode_sequence(batch.frag)
            if hasattr(self.model.encoder, 'transformer'):
                # frag_emb: [seq_len, batch_size, 256]
                frag_avg = frag_emb.transpose(0, 1).mean(dim=1).contiguous()  # [batch, hidden]
                frag_sim = self.model.introspector(frag_avg).unsqueeze(1)
            else:
                frag_avg = frag_memory_bank.transpose(0, 1).mean(dim=1).contiguous()
                frag_sim = self.model.introspector(frag_avg).unsqueeze(1)
            self.model.log_fragment_scores(frag_sim, frag_lengths.size(0))
        # Handle case when src_lengths is None
        if src_lengths is None:
            assert not isinstance(
                memory_bank, tuple
            ), "Ensemble decoding only supported for text data"
        ...
        return src, enc_states, memory_bank, src_lengths, ref_memory_bank, ref_lengths, frag_memory_bank, frag_lengths, frag_sim
    ...
    def _translate_batch_with_strategy(self, batch, src_vocabs, decode_strategy):
        """Translate batch using specified search strategy"""
        use_src_map = self.copy_attn
        ...
        # Repeat src objects for beam size
        ref_memory_bank, ref_memory_lengths, frag_memory_bank, frag_memory_lengths, sim, frag_sim = (
            tile(ref_memory_bank, decode_strategy.beam_size, dim=1) if ref_memory_bank is not None else None,
            tile(ref_lengths, decode_strategy.beam_size) if ref_lengths is not None else None,
            tile(frag_memory_bank, decode_strategy.beam_size, dim=1) if frag_memory_bank is not None else None,
            tile(frag_lengths, decode_strategy.beam_size) if frag_lengths is not None else None,
            tile(batch.sim, decode_strategy.beam_size) if batch.sim is not None else None,
            tile(frag_sim, decode_strategy.beam_size) if frag_sim is not None else None,
        )
        ...
        for step in range(decode_strategy.max_length):
            ...
            log_probs, attn = self._decode_and_generate(
                ...
                batch_offset=decode_strategy.batch_offset,
                ref_memory_bank=ref_memory_bank,
                ref_memory_lengths=ref_memory_lengths,
                sim=sim,
                frag_memory_bank=frag_memory_bank,
                frag_memory_lengths=frag_memory_lengths,
                frag_sim=frag_sim,
            )
            decode_strategy.advance(log_probs, attn)
            any_finished = decode_strategy.is_finished.any()
            ...
            if any_finished:
                def reorder_seq_and_len(seq, lengths):
                    if isinstance(seq, tuple):
                        return tuple(x.index_select(1, select_indices) for x in seq), lengths.index_select(0,
                                                                                                           select_indices)
                    else:
                        return seq.index_select(1, select_indices), lengths.index_select(0, select_indices)

                memory_bank, memory_lengths = reorder_seq_and_len(memory_bank, memory_lengths)
                ref_memory_bank, ref_memory_lengths = reorder_seq_and_len(ref_memory_bank, ref_memory_lengths) \
                    if ref_memory_bank is not None else (None, None)
                frag_memory_bank, frag_memory_lengths = reorder_seq_and_len(frag_memory_bank, frag_memory_lengths) \
                    if frag_memory_bank is not None else (None, None)
                sim = sim.index_select(0, select_indices) if sim is not None else None
                frag_sim = frag_sim.index_select(0, select_indices) if frag_sim is not None else None
                if src_map is not None:
                    src_map = src_map.index_select(1, select_indices)

            if parallel_paths > 1 or any_finished:
                self.model.decoder.map_state(  # Reorder decoder states
                    lambda state, dim: state.index_select(dim, select_indices)
                )

        return self.report_results(
            gold_score, batch, batch_size, src, src_lengths,
            src_vocabs, use_src_map, decode_strategy
        )
    ...
    def _decode_and_generate(
            ...
            ref_memory_lengths=None,
            sim=None,
            frag_memory_bank=None,
            frag_memory_lengths=None,
            frag_sim=None,
    ):
        """Decode and generate tokens, including reference handling"""
        if self.copy_attn:
            decoder_input = decoder_input.masked_fill(
                decoder_input.gt(self._tgt_vocab_len - 1), self._tgt_unk_idx
            )
    ...
    def _score_target(
            self, batch, memory_bank, src_lengths, src_vocabs, src_map,
            ref_memory_bank=None, ref_lengths=None, sim=None,
            frag_memory_bank=None, frag_lengths=None, frag_sim=None
    ):
        """Score gold targets using reference if available"""
        tgt = batch.tgt
        tgt_in = tgt[:-1]
        # Decode and generate log probabilities
        log_probs, attn = self._decode_and_generate(
            tgt_in,
            memory_bank,
            batch,
