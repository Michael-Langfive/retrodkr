...
class TranslationBuilder(object):
    ...
    def from_batch(self, translation_batch):
        ...
        if hasattr(batch, 'ref'):
            ref_tensor, _ = batch.ref
        else:
            ref_tensor = None
        if hasattr(batch, 'frag'):
            frag_tensor, _ = batch.frag
        else:
            frag_tensor = None
        ref = ref_tensor[:, :, 0].index_select(1, perm) if hasattr(batch, "ref") else None
        frag = frag_tensor[:, :, 0].index_select(1, perm) if hasattr(batch, "frag") else None
        sim = batch.sim.index_select(0, perm) if hasattr(batch, "sim") else None
        ...
        for b in range(batch_size):
            ...
            ref_sent = None
            if ref is not None:
                ref_sent = self._build_target_tokens(
                    src[:, b] if src is not None else None,
                    src_vocab,
                    src_raw,
                    ref[1:, b] if ref is not None else None,
                    None,
                )
            frag_sent = None
            if frag is not None:  # 将token转换为文本
                frag_sent = self._build_target_tokens(
                    src[:, b] if src is not None else None,
                    src_vocab,
                    src_raw,
                    frag[1:, b] if frag is not None else None,
                    None,
                )
            sim_score = None
            if sim is not None:
                sim_score = sim[b].item()  # 假设 sim 是一个标量值
...
class Translation(object):
    ...
    __slots__ = ["src", "src_raw", "pred_sents", "attns", "pred_scores",
                 "gold_sent", "gold_score", "word_aligns", "ref_sent", "sim_score", "frag_sent"]

    def __init__(self, src, src_raw, pred_sents, attn, pred_scores,
                 tgt_sent, gold_score, word_aligns, ref_sent, sim_score, frag_sent):
        self.src = src
    ...
    def log(self, sent_number):
        ...
        # 添加对参考序列的显示
        if self.ref_sent is not None:
            ref_sent = ' '.join(self.ref_sent)
            msg.append('REF {}: {}\n'.format(sent_number, ref_sent))
        # 添加对相似度分数的显示
        if self.sim_score is not None:
            msg.append("SIM SCORE: {:.4f}\n".format(self.sim_score))
        if self.frag_sent is not None:
            frag_sent = ' '.join(self.frag_sent)
            msg.append('FRAG {}: {}\n'.format(sent_number, frag_sent))
