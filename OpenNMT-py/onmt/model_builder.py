...
def build_task_specific_model(model_opt, fields):
    ...
    if model_opt.share_embeddings:
        ...
        if model_opt.model_task == ModelTask.RetroDKR:
            assert (
                    ...
                    == fields["ref"].base_field.vocab == fields["frag"].base_field.vocab
            ), "preprocess with -share_vocab if you use share_embeddings with RetroDKR task"
        else:
            assert (
                    fields["src"].base_field.vocab == fields["tgt"].base_field.vocab
            ), "preprocess with -share_vocab if you use share_embeddings"
    logger.info(f"build {model_opt.model_task} model")
    ...
    elif model_opt.model_task == ModelTask.RetroDKR:  # 新增任务
        encoder, src_emb = build_encoder_with_embeddings(model_opt, fields)
...
def build_base_model(model_opt, fields, gpu, checkpoint=None, gpu_id=None):
    ...
    if not model_opt.copy_attn:
        ...
        generator = nn.Sequential(  # 将 nn.Sequential 用于生成器
            nn.Linear(model_opt.dec_rnn_size,
                      len(fields["tgt"].base_field.vocab)),
            Cast(torch.float32),
            gen_func  # 使用指定的生成函数 nn.LogSoftmax或LogSparsemax层
        )
