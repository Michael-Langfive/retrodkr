...
class TransformPipe(Transform):
    ...
    def get_specials(cls, opts, transforms):
        ...
        src_specials, tgt_specials, ref_specials, frag_specials = set(), set(), set(), set()
        for transform in transforms:
            _src, _tgt, _ref, _frag = transform.get_specials(transform.opts)  # 解包三元组
            src_specials.update(_src)
            tgt_specials.update(_tgt)
            if opts.data_task == ModelTask.RetroDKR:
                ref_specials.update(_ref)
                frag_specials.update(_frag)
        return src_specials, tgt_specials, ref_specials, frag_specials
...
def get_specials(opts, transforms_cls_dict):
    """Get specials of transforms that should be registered in Vocab."""
    all_specials = {'src': set(), 'tgt': set()}
    if opts.data_task == ModelTask.RetroDKR:
        all_specials['ref'] = set()
        all_specials['frag'] = set()

    for name, transform_cls in transforms_cls_dict.items():
        # Assuming transform_cls.get_specials can return ref_specials
        src_specials, tgt_specials, ref_specials, frag_specials = transform_cls.get_specials(opts)
        all_specials['src'].update(src_specials)
        all_specials['tgt'].update(tgt_specials)
        if opts.data_task == ModelTask.RetroDKR:
            all_specials['ref'].update(ref_specials)
            all_specials['frag'].update(frag_specials)
