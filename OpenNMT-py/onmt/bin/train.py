...
def prepare_fields_transforms(opt):
    ...
    fields = build_dynamic_fields(
        opt, src_specials=specials['src'],
        tgt_specials=specials['tgt'],
        ref_specials=specials['ref'] if 'ref' in specials else None,
        frag_specials=specials['frag'] if 'frag' in specials else None,
    )
...
def _init_train(opt):
    ...
    for side in ['src', 'tgt', 'ref', 'frag']:
        if side in fields:  # 添加检查，因为某些任务可能没有ref字段
            f = fields[side]
            try:
                f_iter = iter(f)
            except TypeError:
