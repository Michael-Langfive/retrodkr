...
def _add_dynamic_fields_opts(parser, build_vocab_only=False):
    ...
    group.add("-tgt_vocab", "--tgt_vocab",
              help=("Path to save" if build_vocab_only else "Path to")
                   + " tgt vocabulary file. "
                     "Format: one <word> or <word>\t<count> per line.")
    group.add("-ref_vocab", "--ref_vocab",
              help=("Path to save" if build_vocab_only else "Path to")
                   + " ref vocabulary file. "
                     "Format: one <word> or <word>\t<count> per line.")
    group.add("-frag_vocab", "--frag_vocab",
              help=("Path to save" if build_vocab_only else "Path to")
                   + " frag vocabulary file. "
                     "Format: one <word> or <word>\t<count> per line.")
    ...
    group.add("-data_task", "--data_task",
              default=ModelTask.SEQ2SEQ,
              choices=[ModelTask.SEQ2SEQ, ModelTask.LANGUAGE_MODEL, ModelTask.RetroDKR],
              help="Type of task for the data: seq2seq, lm, or RetroDKR")
    if not build_vocab_only:
        ...
        group.add("-ref_vocab_size", "--ref_vocab_size",
                  type=int, default=50000,
                  help="Maximum size of the reference vocabulary.")
        group.add("-frag_vocab_size", "--frag_vocab_size",
                  type=int, default=50000,
                  help="Maximum size of the fragment vocabulary.")
        ...
        group.add("-ref_words_min_frequency", "--ref_words_min_frequency",
                  type=int, default=0,
                  help="Discard reference words with lower frequency.")
        group.add("-frag_words_min_frequency", "--frag_words_min_frequency",
                  type=int, default=0,
                  help="Discard fragment words with lower frequency.")
        ...
        group.add("--ref_seq_length_trunc", "-ref_seq_length_trunc",
                  type=int, default=None,
                  help="Truncate reference sequence length.")
        group.add("--frag_seq_length_trunc", "-frag_seq_length_trunc",
                  type=int, default=None,
                  help="Truncate fragment sequence length.")

        group = parser.add_argument_group('Embeddings')
        group.add('-all_embeddings', '--all_embeddings',
                  help="Path to the embeddings file to use "
                       "for all source, target and ref tokens.")
        ...
        group.add('-ref_embeddings', '--ref_embeddings',
                  help="Path to the embeddings file to use for reference tokens.")
        group.add('-frag_embeddings', '--frag_embeddings',
                  help="Path to the embeddings file to use for fragment tokens.")
...
def model_opts(parser):
    ...
    group.add('--ref_word_vec_size', '-ref_word_vec_size',
              type=int, default=500,
              help='Word embedding size for ref.')
    group.add('--frag_word_vec_size', '-frag_word_vec_size',
              type=int, default=500,
              help='Word embedding size for frag.')
    group.add('--word_vec_size', '-word_vec_size', type=int, default=-1,
              help='Word embedding size for src, tgt, ref and frag.')
    ...
    group.add(
        "-model_task", "--model_task", default=ModelTask.SEQ2SEQ,
        choices=[ModelTask.SEQ2SEQ, ModelTask.LANGUAGE_MODEL, ModelTask.RetroDKR],
        help="Type of task for the model: seq2seq, lm, or RetroDKR"
    )
    group.add(
        "-lambda1", "--lambda1", type=float,
        help="First lambda parameter for RetroDKR task (required if model_task is RetroDKR)"
    )
    group.add(
        "-lambda2", "--lambda2", type=float,
        help="Second lambda parameter for RetroDKR task (required if model_task is RetroDKR)"
    )
    group.add(
        "-lambda3", "--lambda3", type=float,
        help="Third lambda parameter for RetroDKR task (required if model_task is RetroDKR)"
    )
    ...
    group.add('--decoder_type', '-decoder_type', type=str, default='rnn',
              choices=['rnn', 'transformer', 'cnn', 'transformer_lm', 'RetroDKR'],
...
def translate_opts(parser):
    ...
    group.add('--ref', '-ref',
              help='Reference sequence (optional)')
    group.add('--sim', '-sim',
              help='Similarity score (optional)')
    group.add('--frag', '-frag',
              help='Fragment sequence (optional)')
