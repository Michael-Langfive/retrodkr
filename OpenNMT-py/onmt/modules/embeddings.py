...
def prepare_pretrained_embeddings(opt, fields):
    if all([opt.all_embeddings is None,
            opt.src_embeddings is None,
            opt.tgt_embeddings is None,
            opt.ref_embeddings is None,
            ]):
        return
    ...
    vocs = []
    for side in ['src', 'tgt', 'ref']:
        try:
            vocab = fields[side].base_field.vocab
        except AttributeError:
            vocab = fields[side].vocab
        vocs.append(vocab)
    enc_vocab, dec_vocab, ref_vocab = vocs

    skip_lines = 1 if opt.embeddings_type == "word2vec" else 0
    # Step 1: Load vectors from file(s)
    if opt.all_embeddings is not None:
        merged_vocab = set(enc_vocab.stoi.keys()) | set(dec_vocab.stoi.keys()) | set(ref_vocab.stoi.keys())
        logger.info("Reading all embeddings from {}".format(opt.all_embeddings))
        all_vectors, total_vec_count = read_embeddings(opt.all_embeddings, skip_lines, merged_vocab)
        src_vectors = tgt_vectors = ref_vectors = all_vectors
        logger.info("\tFound {} total vectors in file.".format(total_vec_count))
    else:
        src_vectors = tgt_vectors = ref_vectors = None
        ...
        if opt.tgt_embeddings:
            ...
            logger.info("\tFound {} total vectors in file.".format(total_vec_count))
        if opt.ref_embeddings:
            logger.info("Reading reference embeddings from {}".format(opt.ref_embeddings))
            ref_vectors, total_vec_count = read_embeddings(opt.ref_embeddings, skip_lines, filter_set=ref_vocab.stoi)
            logger.info("\tFound {} total vectors in file.".format(total_vec_count))

    logger.info("After filtering to vectors in vocab:")
    if opt.src_embeddings is not None or opt.all_embeddings is not None:
        logger.info("\t* enc: %d match, %d missing, (%.2f%%)"
                    % calc_vocab_load_stats(enc_vocab, src_vectors))
    if opt.tgt_embeddings is not None or opt.all_embeddings is not None:
        logger.info("\t* dec: %d match, %d missing, (%.2f%%)"
                    % calc_vocab_load_stats(dec_vocab, tgt_vectors))
    if ref_vectors is not None:
        logger.info("\t* ref: %d match, %d missing, (%.2f%%)" %
                    calc_vocab_load_stats(ref_vocab, ref_vectors))

    # Step 3: Save embeddings
    if src_vectors:
        enc_out_file = opt.save_data + ".enc_embeddings.pt"
        logger.info("Saving encoder embeddings to:\n\t* %s" % enc_out_file)
        torch.save(convert_to_torch_tensor(src_vectors, enc_vocab), enc_out_file)
        opt.pre_word_vecs_enc = enc_out_file

    if tgt_vectors:
        dec_out_file = opt.save_data + ".dec_embeddings.pt"
        logger.info("Saving decoder embeddings to:\n\t* %s" % dec_out_file)
        torch.save(convert_to_torch_tensor(tgt_vectors, dec_vocab), dec_out_file)
        opt.pre_word_vecs_dec = dec_out_file

    if ref_vectors:
        ref_out_file = opt.save_data + ".ref_embeddings.pt"
        logger.info("Saving reference embeddings to:\n\t* %s" % ref_out_file)
        torch.save(convert_to_torch_tensor(ref_vectors, ref_vocab), ref_out_file)
        opt.pre_word_vecs_ref = ref_out_file