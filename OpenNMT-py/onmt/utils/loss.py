...
def build_loss_compute(model, tgt_field, opt, train=True):
    ...
    if opt.copy_attn:
        ...
        elif opt.model_task == ModelTask.RetroDKR:
            compute = onmt.modules.CopyGeneratorRetroDKRLossCompute(
                criterion, loss_gen, tgt_field.vocab,
                opt.copy_loss_by_seqlength,
                lambda_coverage=opt.lambda_coverage
            )
...
class RetroDKRLossCompute(CommonLossCompute):
    """Search Retro Loss Computation."""

    def __init__(self, criterion, generator, normalization="sents",
                 lambda_coverage=0.0, lambda_align=0.0):
        super(RetroDKRLossCompute, self).__init__(criterion, generator,
                                            normalization=normalization,
                                            lambda_coverage=lambda_coverage,
                                            lambda_align=lambda_align,
                                            tgt_shift_index=1)
