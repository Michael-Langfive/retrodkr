...
class DataOptsCheckerMixin(object):
        ...
        for cname, corpus in corpora.items():
            ...
            if path_src is None:
                ...
                if path_tgt is None:
                    ...
                    if opt.data_task != ModelTask.RetroDKR:  # 如果不是RetroDKR，才考虑设置为LANGUAGE_MODEL
                        opt.data_task = ModelTask.LANGUAGE_MODEL
                    # tgt is src for LM task
                    corpus["path_tgt"] = path_src
                    corpora[cname] = corpus
                    path_tgt = path_src
                    if opt.data_task == ModelTask.RetroDKR:
                        path_ref = corpus.get('path_ref', None)
                        path_sim = corpus.get('path_sim', None)
                        if path_ref is None:
                            raise ValueError(f'Corpus {cname} ref path is required for RetroDKR task.')
                        if path_sim is None:
                            raise ValueError(f'Corpus {cname} sim path is required for RetroDKR task.')
                        cls._validate_file(path_ref, info=f'{cname}/path_ref')  # 验证文件实际存在并且可访问
                        cls._validate_file(path_sim, info=f'{cname}/path_sim')
    ...
    def _validate_fields_opts(cls, opt, build_vocab_only=False):
        ...
        if not opt.share_vocab:
            ...
            if hasattr(opt, 'ref_vocab') and opt.ref_vocab:
                cls._validate_file(opt.ref_vocab, info='ref vocab')
        ...
        if opt.all_embeddings is not None:
            if hasattr(opt, 'ref_embeddings'):
                assert (opt.src_embeddings is None
                        and opt.tgt_embeddings is None
                        and opt.ref_embeddings is None), \
                    "You don't need -src_embeddings, -tgt_embeddings or -ref_embeddings \
                    if -all_embeddings is set."
            else:
                assert (opt.src_embeddings is None
                        and opt.tgt_embeddings is None), \
                    "You don't need -src_embeddings or -tgt_embeddings \
                    if -all_embeddings is set."
        # 检查是否有ref_embeddings属性
        has_embeddings = [opt.all_embeddings is not None,
                          opt.src_embeddings is not None,
                          opt.tgt_embeddings is not None]
        if hasattr(opt, 'ref_embeddings'):
            has_embeddings.append(opt.ref_embeddings is not None)
